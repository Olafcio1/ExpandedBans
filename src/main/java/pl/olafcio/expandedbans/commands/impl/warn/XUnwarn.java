...
public class XUnwarn extends XTargetCommand {
    ...
    protected void apply(CommandSender sender, Command command, String label, List<Object> args) throws SQLException, CommandMessageException {
        ...
        if (!ExpandedBans.Database.removeWarns(target, reason))
            throw new CommandMessageException("§6%s§4 is not warned.".formatted(player.getName()));

        if (reason == null) {
            ExpandedBans.Messages.send(sender,
                    "§7Unwarned §6%s§7.".formatted(player.getName()));
        } else {
            ExpandedBans.Messages.send(sender,
                    "§7Unwarned §6%s§7 with the reason §o%s.".formatted(player.getName(), reason));
        }
    }
